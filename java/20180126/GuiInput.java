...
public class GuiInput extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		String cmd = e.getActionCommand();
		if (cmd.equals("Clear")) for (int i=0; i<this.jtf.length; i++) this.jtf[i].setText("");
		else if (cmd.equals("Ok")) {
			String[] data = new String[this.jtf.length];
			for (int i=0; i<this.jtf.length; i++) data[i] = this.jtf[i].getText();
			addData(data);
		}
	}
	private void addData(String[] str){
		try{
			BufferedWriter bw = new BufferedWriter(new FileWriter(new File("data.txt"), true));
			bw.write(toString(str)+"\r\n");
			bw.close();
		}catch(IOException e){
